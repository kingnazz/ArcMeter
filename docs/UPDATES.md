...
Updater signatures are separate from Windows Authenticode signing and Apple Developer ID signing/notarization. Production releases should use both layers: Tauri updater signatures protect the update channel, while operating-system signing establishes publisher identity and avoids Windows SmartScreen or macOS Gatekeeper warnings.
...
Apple and Windows platform-signing secrets can be added when those certificates are available. Do not put a Supabase service-role key in a desktop build or in these client configuration values.
