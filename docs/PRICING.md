...
Grok events remain `pricing_status = unavailable` in V1 because no exact official static rate mapping was safely established for the observed Grok Build model identifiers. Unknown models and events before a rule's effective date also remain unavailable. ArcMeter sums only events with an exact pricing match. When a selected range also contains unavailable events, the UI marks the result as a lower-bound partial estimate and shows the share of measured tokens covered by safe pricing instead of guessing a rate for the remainder.
