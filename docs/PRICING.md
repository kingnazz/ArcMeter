...
Rates are stored as integer USD micros per million tokens. Context tiers are separate rows keyed by provider, model pattern, effective date, minimum input-token boundary, and version. A rule records whether cache counters are included in `input_tokens` or additive. Included counters are subtracted before ordinary input pricing; additive counters are priced alongside fresh input. Reasoning is not added twice when the provider reports it as part of output.

Claude pricing includes the official 0.1x cache-read, 1.25x 5-minute cache-write, and 2x 1-hour cache-write rates. Claude records with only aggregate cache creation receive a `partial` safe subtotal because assigning an unknown cache lifetime would be a guess. The pricing context boundary uses total provider input (fresh + cache read + cache write for Claude). Current Claude 4.6+ catalog rules use standard pricing throughout the documented 1M window; cataloged 4.5 families stop at 200k.
